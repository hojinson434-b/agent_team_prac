...
13. **params/searchParams는 항상 문자열** — `useParams()`, `useSearchParams()`가 반환하는 값은 무조건 문자열이다. 숫자 ID와 비교할 때 반드시 `Number(params.id)`로 변환한 뒤 비교할 것. `p.id === params.id`처럼 직접 비교하면 타입 불일치로 항상 `false`가 된다.
14. **페이지 생성 시 진입 링크 전수 검사** — 새 페이지를 `app/xyz/page.js`에 만들면, 해당 페이지로 연결되는 모든 `<Link href>`를 Grep으로 검색해서 경로가 일치하는지 확인한다. 예: `/auth`에 페이지를 만들었으면 `href="/login"` 같은 잘못된 경로가 없는지 `Grep "href.*login"` 등으로 확인.
15. **수정 후 사용자 동선 검증** — 기능을 수정하면, 사용자가 그 기능에 도달하는 전체 경로를 따라간다. 예: 로그인 기능 수정 → (1) 헤더에서 로그인 버튼 클릭 (2) 로그인 페이지 렌더링 확인 (3) 폼 제출 후 리다이렉트 확인. Context만 고치고 진입 경로를 확인하지 않으면 사용자에게는 여전히 "안 됨"이다.
16. **같은 문제 두 번 말하게 하지 않기** — 사용자가 보고한 문제를 수정할 때, 해당 문제와 관련된 모든 파일을 한 번에 점검한다. 예: "로그인이 안 보인다" → AuthContext, auth 페이지, Header의 로그인 링크, MobileMenu의 로그인 링크를 모두 확인한 뒤 수정. 한 곳만 고치고 나머지를 빠뜨리면 사용자가 같은 피드백을 반복해야 한다.
